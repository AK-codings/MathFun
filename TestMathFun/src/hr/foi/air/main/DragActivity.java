...
public class DragActivity extends BaseActivity implements OnTouchListener, OnClickListener, OnGlobalLayoutListener {

	final short NO_POSITION = -1;
	int numOfQuestions = 10;
	int levelNo = 1;
	boolean finished = false;
	SecureRandom sRnd = new SecureRandom();
	List<Rect> registeredLocs = new ArrayList<Rect>();
	List<Rect> initialPositions =new ArrayList<Rect>();
	
	short questionNum = 0;
	Questions[] questions = new Questions[1];
	
	boolean isTaskPositioned = false;
	RelativeLayout layout = null;
	int previousRawX = 0;
	int previousRawY = 0;
	int initialBitmapSize = 0;
	int middleLayVertCenter = 0;
	
	boolean goneOutLeft = false;
	boolean goneOutRight = false;
	int goneOutX = -1;
	boolean goneOutTop = false;
	boolean goneOutDown = false;
	int goneOutY = -1;
	
	Timer timer= null;
	TextView tvPassedTime = null;
	int secondsPassed = 0;
	String passedTimeString = null;
	
	int numOfCorrectAnswers = 0;
	
	List<ObjToShow> questionChars = null;
	...
	private void setQuestionsSet(){
		ArrayList<Questions> questionList = Question_generator.generate(numOfQuestions, levelNo);
	...
	private void setPreview(){
		...
		tvLevelNo.setText(getResources().getString(R.string.Razina) + " " + Integer.toString(levelNo));
		TextView tvQuestionNo = (TextView)findViewById(R.id.tvQuestionNum);
		tvQuestionNo.setText(getResources().getString(R.string.questionNum) + " " + Integer.toString(questionNum + 1) + "/" + Integer.toString(questions.length));
		...
		short index = 0;
		for(Iterator<ObjToShow> iter = questionChars.iterator(); iter.hasNext();){
			MyImageView image = new MyImageView(this);
			ObjToShow o = iter.next();
			Bitmap bmp = createBitmapFromText(o.text, initialBitmapSize, Color.RED);
			...
			{
				registeredLocs.add(new Rect(startPos+previousX,middleLayVertCenter-(bmpHeight/2),startPos+previousX+bmp.getWidth(),middleLayVertCenter+(bmpHeight/2)));
				int srX =0;
				int srY = 0;
				boolean ok = false;
				while(!ok){
					...
					ok = true;
					if(rlp.topMargin <= middleLayVertCenter+(bmpHeight/2) && rlp.topMargin >= middleLayVertCenter-(bmpHeight/2)){
						ok=false;
					}
					if((rlp.topMargin+bmp.getHeight()) <= middleLayVertCenter+(bmpHeight/2) && (rlp.topMargin+bmp.getHeight()) >= middleLayVertCenter-(bmpHeight/2)){
						ok=false;
					}
					for(Rect r : initialPositions){
						if( ((rlp.leftMargin >= r.left && rlp.leftMargin <= r.right) || (rlp.leftMargin+bmp.getWidth() >= r.left && rlp.leftMargin+bmp.getWidth() <= r.right)) &&((rlp.topMargin >= r.top && rlp.topMargin <= r.bottom) || (rlp.topMargin+bmpHeight >= r.top && rlp.topMargin+bmpHeight <= r.bottom)) ){
							ok = false;
						}
					}
				}
				...
				if(rlp.topMargin < 0)
					rlp.topMargin = 0;
				initialPositions.add(new Rect(rlp.leftMargin,rlp.topMargin,rlp.leftMargin+bmpHeight,rlp.topMargin+bmpHeight));
			}
	...
	public boolean onTouch(View view, MotionEvent event) {
		...
		switch (event.getAction() & MotionEvent.ACTION_MASK) {
			...
			case MotionEvent.ACTION_UP:
				...
				if(miv1 != null){
					RelativeLayout.LayoutParams rlp = (RelativeLayout.LayoutParams)miv1.getLayoutParams();
					short index = 0;
					for(Iterator<Rect> iter = registeredLocs.iterator();iter.hasNext();){
						Rect r = iter.next();
						if(( (rlp.leftMargin >= r.left && rlp.leftMargin <= r.right) || (rlp.leftMargin+miv1.getWidth() >= r.left && rlp.leftMargin+miv1.getWidth() <= r.right) ) && ( (rlp.topMargin >= r.top && rlp.topMargin <= r.bottom) || (rlp.topMargin+miv1.getHeight() >= r.top && rlp.topMargin+miv1.getHeight() <= r.bottom) )){
			...
			case MotionEvent.ACTION_MOVE:
				...
				if(!goneOutLeft && !goneOutRight){
					int parentWidth = ((RelativeLayout)view.getParent()).getWidth();
					...
					else if(layoutParams.leftMargin > (parentWidth-view.getWidth())){
						goneOutRight = true;
						goneOutX = X - ((layoutParams.leftMargin+view.getWidth()) - parentWidth);
				...
				if(!goneOutTop && !goneOutDown){
					...
					int parentHeight = ((RelativeLayout)view.getParent()).getHeight();
					...
					else if(layoutParams.topMargin > (parentHeight-view.getHeight())){
						goneOutDown = true;
						goneOutY = Y - ((layoutParams.topMargin+view.getHeight()) - parentHeight);
	 ...
	 * @param index location index to check
	...
	private List<ObjToShow> setQuestionOnIndex(short index){
		...
		if(numsLength > charsLength){
			...
			for(int n : nums){
				...
				if(i<charsLength){
					questionObjects.add(new ObjToShow(Character.toString(chars[i]),true));
					++i;
				}
			}
		}
	 ...
	 * @param text text to write on bitmap
	 * @param textSize size of text to show
	 * @param textColor color of text to show
	 * @return bitmap with text and color
	 */
	private Bitmap createBitmapFromText(String text, float textSize, int textColor) {
    	Typeface tf = Typeface.create(Typeface.SANS_SERIF, Typeface.BOLD);
        Paint paint = new Paint();
        paint.setTypeface(tf);
        paint.setTextSize(textSize);
        ...
        Bitmap image = Bitmap.createBitmap(height, height, Bitmap.Config.ARGB_8888);
        if(text.length()>2)
        {
				...
				runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						if(sec<10)
							sb.append("0");
						sb.append(sec);
						tvPassedTime.setText(passedTimeString + " " + Integer.toString(min) + ":" + sb.toString());
					}
				});
			}
		}, (long) (1 * 1000), 1 * 1000);
	 ...
	 * @param numbersToCheck list of objects to check
	...
	void showResults(){
		...
		intent.putExtra("brojTocnihOdgovora", numOfCorrectAnswers * 1000 / secondsPassed);
